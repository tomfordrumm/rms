...
const workingHoursPrimary = computed(
    () => props.restaurant.work_hours?.trim() || null,
);
...
const visibleDishes = computed(
    () => selectedCategory.value?.dishes ?? allDishes.value,
);
const cartStorageKey = computed(
    () => `restaurant-menu-cart:${props.restaurant.slug}`,
);
const storedCart = useStorage<Record<string, CartItem[]>>(
    'restaurant-menu-carts',
    {},
);
...
function mergeCartItems(
    itemsToMerge: Array<DishPayload & { quantity: number }>,
): void {
...
async function submitMagicOrder(): Promise<void> {
    if (
        magicOrderPreferences.value.trim() === '' ||
        isMagicOrderLoading.value
    ) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(
            `/r/${props.restaurant.slug}/magic-order`,
            {
                method: 'POST',
                headers: {
...
<template>
    ...
    >
        <Dialog
            :open="isMagicOrderOpen"
            @update:open="
                (open) => (open ? openMagicOrder() : closeMagicOrder())
            "
        >
            <DialogContent
            ...
            >
                <DialogHeader
                    class="border-b border-stone-200/80 px-6 py-6 text-left sm:px-8"
                >
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <DialogDescription class="text-slate-500">
                                Tell us what you want and we will assemble a
                ...
                <div class="space-y-6 px-6 py-6 sm:px-8 sm:py-8">
                    <div v-if="magicOrderResult" class="space-y-5">
                        <div
                            class="rounded-[1.5rem] border border-stone-200 bg-white p-5 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)]"
                        >
                            <div
                                class="mb-3 flex items-center gap-2 text-sm font-semibold tracking-[0.2em] text-slate-500 uppercase"
                            >
                            ...
                            >
                                ...
                                >
                                    <div class="min-w-0">
                                        ...
                                        <p
                                            class="mt-1 text-xs tracking-[0.18em] text-slate-400 uppercase"
                                        >
                                            {{ item.weight }} · Qty
                                            {{ item.quantity }}
                                        </p>
                                    ...
                                    <span
                                        class="shrink-0 text-lg font-extrabold text-emerald-600"
                                    >
                    ...
                    <div v-else class="space-y-5">
                        <div class="space-y-3">
                            ...
                            <textarea
                                ...
                                class="min-h-40 w-full rounded-[1.5rem] border border-stone-200 bg-white px-5 py-4 text-sm leading-6 text-slate-700 shadow-[0_20px_50px_-40px_rgba(15,23,42,0.35)] transition outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10"
        ...
        <Sheet v-model:open="isCartOpen">
            ...
            >
                <SheetHeader
                    class="border-b border-stone-200/80 px-6 py-6 text-left"
                >
                    <div class="flex items-center gap-3">
                        <div
                            class="flex size-11 items-center justify-center rounded-full bg-[#12192b] text-white shadow-[0_18px_40px_-24px_rgba(15,23,42,0.85)]"
                ...
                <div class="flex min-h-0 flex-1 flex-col">
                        ...
                        >
                            <div class="flex gap-4">
                                ...
                                >
                                    ...
                                    >
                                        <span
                                            class="line-clamp-2 text-xs leading-4 font-semibold text-white"
                                        >
                                            {{ item.name }}
                                        </span>
                                    </div>
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span
                                                class="min-w-10 text-center text-sm font-semibold text-slate-950"
                                            >
                                                {{ item.quantity }}
                                            </span>
                                            <button
                    ...
                    >
                        <div class="max-w-xs text-center">
                            ...
                            <p class="mt-2 text-sm leading-6 text-slate-500">
                                Add dishes from the menu and they will appear
                                here automatically.
                            </p>
                        </div>
                    </div>
                    ...
                    <div
                        class="border-t border-stone-200/80 bg-white/85 px-6 py-5"
                    >
                        <div
                        ...
                        >
                            ...
                            <span
                                class="text-2xl font-extrabold text-slate-950"
                            >
                        ...
                        <p class="text-xs leading-5 text-slate-500">
                            Cart is stored locally in this browser for this
                            restaurant only.
                        </p>
                    </div>
                </div>
        ...
        <div
            class="mx-auto max-w-7xl px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8"
        >
            <section
            ...
            >
                ...
                />
                <div
                    class="absolute inset-0 bg-[linear-gradient(135deg,rgba(17,19,15,0.2),rgba(17,19,15,0.78)_50%,rgba(6,20,12,0.92)_100%)]"
                />
                <div
                    class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(237,196,114,0.32),_transparent_26%)]"
                />
                <div
                    class="absolute inset-0 bg-[radial-gradient(circle_at_bottom_left,_rgba(66,132,97,0.22),_transparent_28%)]"
                />

                <div
                    class="relative px-6 py-7 sm:px-8 sm:py-10 lg:px-12 lg:py-14"
                >
                    <div
                        class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_240px] lg:items-end"
                    >
                        <div class="space-y-6">
                            <Badge
                                variant="secondary"
                                class="w-fit border border-white/15 bg-white/10 px-4 py-2 text-[0.68rem] font-semibold tracking-[0.28em] text-white uppercase"
                            >
                                Signature Menu
                            </Badge>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="max-w-3xl space-y-3">
                                    <h1
                                        class="text-4xl leading-[0.9] font-semibold tracking-[-0.04em] text-white sm:text-5xl lg:text-7xl"
                                        style="
                                            font-family:
                                                'Cormorant Garamond', ui-serif,
                                                Georgia, serif;
                                        "
                                    >
                                        {{ restaurant.name }}
                        ...
                        >
                            <div
                                class="mb-3 flex items-center gap-3 text-sm font-semibold tracking-[0.2em] text-stone-100 uppercase"
                            >
                            ...
                            <div class="space-y-2">
                                <p
                                    ...
                                    class="text-lg leading-7 font-semibold text-white"
                                >
                                    {{ workingHoursPrimary }}
                                </p>
                                <p
            ...
            <main class="mt-8 space-y-8 sm:mt-10 sm:space-y-10">
                ...
                >
                    ...
                    >
                        ...
                        <div
                            class="inline-flex w-fit items-center gap-2 rounded-full border border-stone-200 bg-stone-50 px-4 py-2 text-xs font-semibold tracking-[0.22em] text-slate-500 uppercase"
                        >
                    ...
                    >
                        ...
                        >
                            <div
                                class="relative aspect-[16/11] overflow-hidden bg-[#141712]"
                            >
                                <img
                                    v-if="dish.image_url"
                                    :src="dish.image_url"
                                ...
                                >
                                    <div class="max-w-[12rem]">
                                        <p
                                            class="text-lg leading-6 font-semibold text-white"
                                            style="
                                                font-family:
                                                    'Cormorant Garamond',
                                ...
                                <div
                                    class="absolute top-4 right-4 rounded-full border border-white/15 bg-[#111827]/88 px-3 py-1 text-[0.63rem] font-semibold tracking-[0.22em] text-white uppercase shadow-lg"
                                >
                                    {{ dish.weight }}
                                </div>
                            </div>
                            ...
                            <div class="space-y-5 px-5 py-5">
                                <div class="space-y-3">
                                    ...
                                    >
                                        <h3
                                            class="text-xl leading-6 font-semibold tracking-[-0.03em] text-slate-950"
                                        >
                                            {{ dish.name }}
                                        </h3>
                                        <span
                                            class="shrink-0 text-lg font-extrabold text-emerald-500"
                                        >
                                    ...
                                    >
                                        Seasonal preparation crafted by the
                                        kitchen for today's service.
                                    </p>
                                </div>
                    ...
                    >
                        ...
                        >
                            Switch to another filter or come back later when the
                            kitchen updates the selection.
                        </p>
                    </div>
                </section>
            </main>
        ...
        >
            <span
                class="relative flex size-11 items-center justify-center rounded-full bg-white/10"
            >
                <ShoppingBag class="size-5" />
                <span
                    v-if="cartItemsCount > 0"
                    class="absolute -top-1 -right-1 inline-flex min-w-5 items-center justify-center rounded-full bg-emerald-500 px-1.5 py-0.5 text-[0.65rem] leading-none font-bold text-white"
                >
                    {{ cartItemsCount }}
                </span>
            </span>
